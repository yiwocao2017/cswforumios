...
- (void)setZanInfo:(ZanInfo *)zanInfo {

    _zanInfo = zanInfo;
    
    _photoImageView.userId = _zanInfo.talker;
    
    //--//
    [self.photoImageView sd_setImageWithURL:[NSURL URLWithString:[_zanInfo.photo convertImageUrl]] placeholderImage:USER_PLACEHOLDER_SMALL];
    self.nameLbl.text= _zanInfo.nickname;
    
    self.timeLbl.text = [_zanInfo.talkDatetime convertToDetailDate];
    
    //1.title

    self.titleLbl.text = [NSString stringWithFormat:@"  标题：%@", _zanInfo.postTitle];
